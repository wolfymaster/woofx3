export default class BitSinger {
  ...
  play(): { audioUrl: string } {
    ...
    if (generator.done) {
      this.currentPlayerIdx = this.playRandom
        ? Math.random() * this.templates.length
        : (this.currentPlayerIdx + 1) % this.templates.length;
      this.currentGenerator = null;
      return this.play();
    }
  ...
  *_play(clips: string[]): Generator<{ audioUrl: string }, void, unknown> {
  ...
  private generateStrings(template: string, count: number, padLength = 0) {
    ...
    const placeholder = template.indexOf("{n}") !== -1 ? "{n}" : template.indexOf("{i}") !== -1 ? "{i}" : "_";

    for (let i = 0; i < count; i++) {
      // Format the number with padding if needed
      const formattedNumber = padLength > 0 ? i.toString().padStart(padLength, "0") : i.toString();
