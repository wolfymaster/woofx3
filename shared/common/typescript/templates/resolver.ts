// Resolver pipeline for `{...}` template strings.
//
// Each value passes through three stages: raw → token substitution →
// expression eval → result. Plain strings without `{...}` segments pass
// through unchanged. The expression evaluator is a hand-written safe AST
// walker — no `eval`, no `new Function()`, no access to globals.
//
// This is the shared implementation used by both streamware (alert
// descriptor fields) and woofwoofwoof (chat command response text). Its
// `{...}` bracket syntax is deliberately disjoint from the Go workflow
// engine's `${...}` path-only syntax (see docs/workflow/expressions.md) so
// a string can flow through both without collision.

export type ResolverContext = Record<string, unknown>;

/**
 * Resolve a template value against a render-time context. Numbers and
 * non-string primitives pass through unchanged. Strings are scanned for
 * `{...}` segments — segments are evaluated as expressions, the
 * surrounding text is left literal.
 */
