...
export function mergeConfigWithEnvironment(
  ...
  const toSnakeUpper = (key: string) => key.replace(/([a-z0-9])([A-Z])/g, "$1_$2").toUpperCase();
  ...
  for (const key of Object.keys(configFile) as Array<keyof MainConfigurationFile>) {
    ...
    if (envObj[envKey] !== undefined) {
      ...
      if (typeof currentValue === "number") {
        ...
        result[key] = ["1", "true", "yes", "on"].includes(incoming.toLowerCase());
      } else {
        result[key] = incoming;
      }
    }
 ...
 */
export async function getPid(rootPath: string, applicationName: string): Promise<number> {
  try {
