...
import { MessageBus, Subscription, Handler, SubscribeOptions, NATSConfig, Logger } from './types';
...
export class NATSBackend implements MessageBus {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.logger.info('Connected to NATS', { url: options.servers, name: options.name });
    } catch (error) {
