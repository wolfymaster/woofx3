import { MessageBus, MessageBusConfig, Backend, Logger } from './types';
import { NATSBackend } from './nats-backend';
...
export async function createMessageBus(config: MessageBusConfig): Promise<MessageBus> {
  switch (config.backend) {
    case 'nats':
      const natsBackend = new NATSBackend(config.nats || {}, config.logger);
      await natsBackend.connect();
      return natsBackend;
      
    case 'http':
      const httpBackend = new HTTPBackend(config.http || {}, config.logger);
      return httpBackend;
 ...
 */
export async function fromEnv(logger?: Logger): Promise<MessageBus> {
