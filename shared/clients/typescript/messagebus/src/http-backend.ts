import { MessageBus, Subscription, Handler, SubscribeOptions, HTTPConfig, Logger } from './types';
...
export class HTTPBackend implements MessageBus {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onOpen = () => {
          ...
          this.logger.info('Connected to HTTP message bus', { url });
          resolve();
        };
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      this.logger.info(`Attempting to reconnect (${this.currentRetryCount}/${this.config.maxRetries || '∞'})`);
  ...
  async close(): Promise<void> {
    ...
    this.logger.info('HTTP message bus connection closed');
  }
