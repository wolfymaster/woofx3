...
pub mod treat_service_server {
    ...
    #[async_trait]
    pub trait TreatService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct TreatServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for TreatServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "treat.TreatService";
    impl<T> tonic::server::NamedService for TreatServiceServer<T> {
