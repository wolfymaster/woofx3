...
export interface OverlayTokenResolverOptions {
  /** Cache TTL in milliseconds. Fixed at 30s. */
  ttlMs?: number;
...
/**
 * Mask a token for log output: keep the `ovl_` prefix plus four
 * characters, elide the rest. Never log plaintext tokens.
 */
 ...
 * Ported from streamware/src/overlay/token-resolver.ts — resolver
 * ownership moves to sceneManager, the logic itself is unchanged.
 *
 * Uniformity invariant (no enumeration oracle): revoked, unknown, and
 * transport-error cases all surface as `null` — callers cannot
 * distinguish them. Definitive misses (db answered NOT_FOUND) are
 * negatively cached for the TTL; transport errors are NOT cached so a
 * recovering db-proxy is retried on the next request.
 *
...
export class OverlayTokenResolver {
  ...
  async resolve(token: string): Promise<ResolvedOverlayToken | null> {
    ...
    if (!this.db) {
      // No db-proxy configured: every token is unresolvable.
      return null;
    }
