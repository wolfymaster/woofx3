...
func NewWorkflowEngine(ctx context.Context, config api.WorkflowEngineConfig, nc *nats.Conn) (api.WorkflowEngine, error) {
...
func newTemporalEngine(ctx context.Context, config api.WorkflowEngineConfig, nc *nats.Conn) (api.WorkflowEngine, error) {
...
func newLocalEngine(ctx context.Context, config api.WorkflowEngineConfig, nc *nats.Conn) (api.WorkflowEngine, error) {
...
func FromEnv(ctx context.Context, eventRepo core.EventRepository, workflowRepo core.WorkflowDefinitionRepository, nc *nats.Conn, logger log.Logger) (api.WorkflowEngine, error) {
        config := api.WorkflowEngineConfig{
                Engine:       getEnvWithDefault("WORKFLOW_ENGINE", "local"),
                TaskQueue:    getEnvWithDefault("WORKFLOW_TASK_QUEUE", "workflow"),
                Logger:       logger,
                EventRepo:    eventRepo,
                WorkflowRepo: workflowRepo,
                Temporal: api.TemporalConfig{
                        Host:      getEnvWithDefault("TEMPORAL_HOST", "localhost:7233"),
                        Namespace: getEnvWithDefault("TEMPORAL_NAMESPACE", "default"),
                },
                Local: api.LocalConfig{
