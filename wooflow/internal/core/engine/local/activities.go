...
func (e *Engine) fetchWorkflowDefinitionActivity(ctx context.Context, params map[string]any) (core.ExecuteActionResult, error) {
	workflowDefID, ok := params["workflow_def_id"].(string)
	if !ok {
		return core.ExecuteActionResult{Success: false, Error: "workflow_def_id parameter required"}, nil
	}

	def, err := e.config.WorkflowRepo.GetWorkflowDefinitionByID(ctx, workflowDefID)
	if err != nil {
		return core.ExecuteActionResult{Success: false, Error: err.Error()}, nil
	}

	return core.ExecuteActionResult{
		Success: true,
...
func (e *Engine) registerWaitingWorkflowActivity(ctx context.Context, params map[string]any) (core.ExecuteActionResult, error) {
	workflowID, ok := params["workflow_id"].(string)
	if !ok {
		return core.ExecuteActionResult{Success: false, Error: "workflow_id parameter required"}, nil
	}

	eventType, ok := params["event_type"].(string)
	if !ok {
		return core.ExecuteActionResult{Success: false, Error: "event_type parameter required"}, nil
	}
	...
	return core.ExecuteActionResult{Success: true}, nil
}

// updateWorkflowStateActivity updates the workflow state
func (e *Engine) updateWorkflowStateActivity(ctx context.Context, params map[string]any) (core.ExecuteActionResult, error) {
	workflowID, ok := params["workflow_id"].(string)
	if !ok {
		return core.ExecuteActionResult{Success: false, Error: "workflow_id parameter required"}, nil
	}
	...
	if !ok {
		return core.ExecuteActionResult{Success: false, Error: "state parameter required"}, nil
	}
...
func (e *Engine) executeAction(ctx context.Context, action string, params map[string]any) (core.ExecuteActionResult, error) {
	e.mu.RLock()
	activity, exists := e.activities[action]
	e.mu.RUnlock()

	if !exists {
		return core.ExecuteActionResult{Success: false, Error: fmt.Sprintf("activity not found: %s", action)}, nil
	}
