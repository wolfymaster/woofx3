...
// HealthCheck verifies connectivity to Temporal server
func (c *Client) HealthCheck(ctx context.Context) error {
        if c.client == nil {
                return fmt.Errorf("temporal client is not initialized")
        }
        
        // Test connectivity using a lightweight operation
        // GetClusterInfo is a simple RPC that confirms server connectivity
        _, err := c.client.WorkflowService().GetClusterInfo(ctx, &workflowservice.GetClusterInfoRequest{})
        if err != nil {
                return fmt.Errorf("temporal server connectivity check failed: %w", err)
        }
