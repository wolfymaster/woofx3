...
| `event` | `string` | Yes | NATS subject to subscribe to. Uses dot notation (e.g., `channel.cheer`). Supports NATS-style wildcards (`*` for one token, `>` for the remaining tail). Same value lives on the underlying trigger row's `event` column. |
