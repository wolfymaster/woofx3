...
func (r *ModuleRepository) UpsertWidget(w *models.Widget) error {
	...
	err := r.db.Raw(`
		INSERT INTO public.widgets (id, name, description, directory, entry, alert_types, accepted_events, settings_schema, surfaces, hosts_surface, created_by_type, created_by_ref, manifest_id, application_id, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
		ON CONFLICT (created_by_type, created_by_ref, manifest_id) WHERE archived_at IS NULL DO UPDATE SET
			name = EXCLUDED.name,
			description = EXCLUDED.description,
			...
			updated_at = NOW()
		RETURNING id
	`, w.ID, w.Name, w.Description, w.Directory, w.Entry, w.AlertTypes, w.AcceptedEvents, w.SettingsSchema, w.Surfaces, w.HostsSurface, w.CreatedByType, w.CreatedByRef, w.ManifestID, w.ApplicationID).Scan(&result).Error
	if err != nil {
		return err
	}
	w.ID = result.ID
