...
func startWorkers(app *types.App) {
	app.PublisherWorker.Start()
	if err := app.AckWorker.Start(); err != nil {
		log.Fatal("failed to start ack worker:", err)
	}
	app.CleanupWorker.Start()
	app.MetricsWorker.Start()
	slog.Info("Workers started", "cache_size", app.EventCache.Size())
}

func stopWorkers(app *types.App) {
	app.PublisherWorker.Stop()
	app.AckWorker.Stop()
	app.CleanupWorker.Stop()
	app.MetricsWorker.Stop()
	slog.Info("Workers stopped", "cache_size", app.EventCache.Size())
}
