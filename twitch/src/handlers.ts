...
import type { ApiClient, HelixChatChatter, HelixUser } from "@twurple/api";
import { z } from "zod";
import type { Context, ErrorHandlerResponse, HandlerResponse, SuccessHandlerResponse } from "./types";
...
export async function moderate(ctx: Context, apiClient: ApiClient, args, messageQueue) {
  ...
  // tools
  const getListOfKnownUsernames = tool(
    async (input) => {
  ...
  const messages = [new SystemMessage(initialSystemMessage), new HumanMessage(message)];
...
export async function complement(apiClient: ApiClient, args) {
  ...
  const messages = [new SystemMessage(initialSystemMessage), new HumanMessage(args.user)];
