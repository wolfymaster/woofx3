import { ApiClient, type HelixEventSubSubscription } from "@twurple/api";
import type {
...
listener.onSubscriptionCreateSuccess((evt: any, sub: HelixEventSubSubscription) => {
  console.log("NOTE THIS ID - YOU NEED IT TO USE THE TWITCH CLI TO TRIGGER EVENTS");
...
listener.onChannelSubscription(userId, async (event: EventSubChannelSubscriptionEvent) => {
  ...
  console.log("triggered channel subscription", userDisplayName, tier, isGift);
...
listener.onChannelSubscriptionGift(userId, async (evt: EventSubChannelSubscriptionGiftEvent) => {
  ...
  console.log("triggered gifted subscription", gifterDisplayName, amount, tier, isAnonymous);
