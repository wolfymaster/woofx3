...
import getNatsClient, { encodePublish } from "./nats";
import type { InvokeRequest, StreamAlertArgs, TimerArgs, TwitchArgs, WebSocketMessage } from "./types";
...
// create NATS client (shared @woofx3/messagebus)
const bus = await getNatsClient();
    ...
    async message(ws, message) {
      try {
        ...
        if (data.type === "invoke") {
          ...
          lua.global.set(
            "httpRequest",
            async (url: string, method: string, opts: { body?: any; headers?: Record<string, string> } = {}) => {
          ...
          lua.global.set("stream_alert", async (args: StreamAlertArgs) => {
            await bus.publish("slobs", encodePublish({ command: "alert_message", args }));
          });

          lua.global.set("twitch", async (args: TwitchArgs) => {
            await bus.publish("twitchapi", encodePublish({ command: "clip", args }));
          });
          ...
          lua.global.set("setCommand", async (args: Command) => {
            ...
            await bus.publish(
              "woofwoofwoof",
              encodePublish({ command: "add_command", args: command })
            );
          });
